// save-x-article.js
// 用法: node save-x-article.js <article_url> <ws_url> <out_dir>
...
async function main() {
  // First open a new tab via browser-level WS, then navigate
  console.log('Opening new tab for X article...');
  const browserWs = new WebSocket(WS_URL);

  await new Promise((resolve, reject) => {
    browserWs.addEventListener('open', resolve);
    browserWs.addEventListener('error', (e) => reject(new Error(e.message || 'WebSocket error')));
  });
  ...
  const bSend = (method, params = {}) => new Promise((resolve, reject) => {
    const id = bMsgId++;
  ...
  // Get the WS debugger URL for the new tab
  await sleep(500);
  ...
  const newTab = tabsRes.find(t => t.id === targetId);
  if (!newTab) throw new Error('Could not find new tab in CDP list');
  const tabWsUrl = newTab.webSocketDebuggerUrl;
  console.log('Tab WS URL:', tabWsUrl);
  browserWs.close();

  // Now connect to the new tab
  console.log('Connecting to new tab CDP...');
  ...
  const waitEvent = (eventName, timeout = 30000) => new Promise((resolve, reject) => {
  ...
  const loadPromise = waitEvent('Page.loadEventFired');
