...
这是没人告诉你的、关于 AI 开发的真相。

你不需要构建自己的模型。你需要构建的只是围绕模型的基础设施：跨 session 持续存在的记忆，规定任务如何完成的技能文件，以及规范 agent 能做什么和不能做什么的协议。
...
四层记忆。内容充实、带有自我改写钩子的技能文件。协议强制执行。一个每晚运行的"梦境循环"（dream loop），将 agent 当天的经验压缩整合。
...
这反映了 Gstack 和 Claude Code 自身记忆钩子的模式。我不是从零设计的，而是通过尝试其他不起作用的布局，逐渐收敛到这个结果。

最关键的洞察——也是 Garry 那条推文让我豁然开朗的地方——就是分离。
...
这个钩子让失败成为学习的契机，而不仅仅是一个错误记录。没有它，失败只是带着通用痛苦评分被记录下来，没有任何反思。
...
我花了大约 30 分钟做了第一个版本。如果你使用 Claude Code 或 Cursor 作为你的 harness，可以完全跳过它，因为它们已经内置了循环、工具调用和文件监视。把它们指向你的 `.agent/` 文件夹，你就完成了。
...
关键函数是 `build_context`，在你看优先级列表之前，它值得先建立一个心智模型。

上下文窗口是一个计算盒子。模型只对其中的内容进行推理。它外面的一切——你的记忆文件、技能库、工具 schema、整个 git 历史——对模型来说都不存在，除非 harness 检索它、塑造它并注入它。

进入上下文窗口的每一块都是一个上下文片段。每个片段都是关于模型现在需要什么才能完成工作的明确决策。

Harness 是一个控制和注入上下文的块系统。有针对性的片段是信号，能引导模型进行更好的计算。冲突或过时的片段是噪音，让模型困惑而不是更聪明。
...
这很重要，原因在于上下文预算。你的模型有固定数量的 token 可以处理。记忆检索、技能加载、工具 schema 和模型自身的推理都在竞争这个预算。

如果你在每次运行时把每个技能文件都倾倒到上下文中，你就在无关指令上浪费 token，模型表现会变差而不是更好。模型对长输入的关注是不均匀的，大上下文中间的信息会被遗漏。
...
我不想夸大这个。Agent 不是有感情的，甚至不是特别聪明。它有的是一致性。它检查自己的笔记，更新自己的指令，不会有坏日子，不会忘记它上周学到的东西。那种一致性，经过几个月的复利，产生了一种感觉上与无状态 agent 本质上不同的东西，尽管底层模型是相同的。

## 构建技能时的苦涩教训

![](images/img-03.jpg)

我在读了 Daniel Miessler 的一条推文后写下了这些。如果你自己构建技能，有一件事值得早点内化：不要写行车路线。写目的地和围栏。
