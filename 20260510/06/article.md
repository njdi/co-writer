...
**Source:** [Meta-Meta-Prompting: The Secret to Making AI Agents Work](https://x.com/Zephyr_hg/status/2053127519872614419)
...
**The skills are fat.** Over 100 of them now, each a self-contained markdown file with detailed instructions for one specific task. Here are a few that ship with GBrain:

- **meeting-ingestion:** After every meeting, it pulls the transcript, creates a structured summary, and then walks through every person and company mentioned and updates their brain pages with what was discussed. The entity propagation back to every person and company page is the real value.
...
- **media-ingest:** Handles video, audio, PDF, screenshots, GitHub repos. Transcribes, extracts entities, files to the right brain location.
...
**The code is fat.** The code that feeds it (scripts for transcription, OCR, social media archival, calendar sync, API integrations) matters too, but the data is where the compound value lives. I run more than 100 crons per day that check all the things.
...
I open-sourced the whole stack. GStack is the coding skill framework (87,000+ stars). GBrain is the knowledge infrastructure. OpenClaw and Hermes Agent are the harnesses.
